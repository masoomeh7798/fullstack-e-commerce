    ...
    return (
        <Stack sx={{ overflowX: 'scroll', '::-webkit-scrollbar': { display: 'none' }, overflow: 'visible' }} alignItems={'start'} direction={'row'} mt={{ xs: 2, md: 3 }} mx={'auto'} gap={3}>
            ...
            <Box  display={{ xs: 'none', md: 'inline-block' }} borderRadius={'8px'} sx={{ position: 'relative' }} >
                ...
                <Box className={`${isOpenCat?'isCatOpen':''}`} sx={{ overflow: 'hidden', border: '1px solid rgba(0,0,0,0.2)', position: 'absolute', top: '90%', right: '0', bgcolor: 'white', textAlign: 'start', '& button': { width: '100%',  justifyContent: 'start', '&:hover': { bgcolor: 'var(--text-clr)' } }, opacity: 0, visibility: 'hidden', transition: 'all .2s ease-out', padding: '8px' }} width={'100%'}>
